...
- [ ] OpenTofu — после уверенного Ansible (не раньше)
- [ ] Docker
- [ ] Kubernetes

---

## Побочная лаба: lab-stack (те же VM)

Отдельный учебный Ansible-проект на **тех же** bastion/app01/db01 — писать inventory/playbook/roles **с нуля**, не копируя этот репозиторий.

| | Этот репозиторий | [lab-stack](https://github.com/KopteloF/lab-stack) |
|--|------------------|-----------------------------------------------------|
| Каталог на bastion | `~/homelab-devops` | `~/lab-stack` |
| App | systemd `homelab-app` :8000 | nginx :8080 |
| db01 | PostgreSQL :5432 | Redis :6379 (host-only bind) |

Оба playbook трогают UFW — не путать правила `8000` (homelab) и `8080`/`6379` (lab-stack).

---

## OpenTofu (отложено)

Создание VM кодом — после того, как Ansible на текущем стенде объясняется уверенно.